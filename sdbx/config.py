...
from typing import ClassVar, Tuple, Type, Union, Literal, List
...
class Config(BaseSettings):
    ...
    model_config: SettingsConfigDict = SettingsConfigDict(env_prefix='SDBX_',)

    def model_post_init(self, __context):
        print(self.path)

        self.model_config["toml_file"] = self.path
    ...
    def get_path_contents(self, name, extension="", path_name=True, base_name=False):
        p = self.get_path(name) if path_name else name

        def format_path(p, g): return os.path.join(p, g)

        if base_name:  # TODO: should the client receive full paths?
            def format_path(_, g): return os.path.basename(g)  # Only return base name
    ...
    def get_path_tree(self, name, extension="", path_name=True, file_callback=(lambda e: e), _searching=None):
        ...
        recurse_tree = partial(self.get_path_tree, name=p,
                               path_name=False, file_callback=file_callback)
        ...
        with os.scandir(os.path.join(p, _searching)) as it:
            for entry in it:
                ...
                tree.append({
                    ...
                    "name": entry.name,
                    **(
                        # If it's a file, load contents as graph
                        file_callback(full) if entry.is_file() else
                        # If it's a directory, recurse
                        {"children": recurse_tree(_searching=current)}
                    )
                })
    ...
    def _path_dict(self):
        root = {
            # see self.location for details
            n: os.path.join(self.path, p) for n, p in dict(self.location).items()
        }

        for _, p in dict(self.location).items():
            if ".." in p:
                raise Exception("Cannot set location outside of config path.")

        models = {f"models.{name}": os.path.join(
            root["models"], name) for name in self.get_default("directories", "models")}
    ...
    def _defaults_dict(self):
        ...
        for filename in glob_source("*.toml") + glob_source("*.json"):
            ...
            ext = filepath.suffix
            loader, mode = (tomllib.load, "rb") if ext == ".toml" else (
                json.load, "r")
    ...
    def node_manager(self):
        # we must import this here lest we summon the dreaded ouroboros
...
def parse() -> Config:
    ...
    parser.add_argument('-c', '--config', type=str,default=get_config_location(), help='Location of the directory containing the config file.')
    ...
    return Config(os.path.join(args.config, filename))

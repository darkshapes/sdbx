...
class Executor:
    ...
    async def execute_node(self, node_id: str, node: Node, inputs: Dict[str, Any]) -> None:
        ...
        :param inputs: Arguments collected from the node widgets to feed into the function
        """
        ...
        if nf.generator:  # Ensure node_function (ie: `nf`) is a generator
            for result in g:  # Iterate over generator `g` values, streaming the results
                await send_result(result)
        else:
            await send_result(g)  # Or just send the final result

    async def process_node(self, graph: nx.MultiDiGraph, node: dict) -> None:
        """Store node execution `result` as tuple within task context `context.results` under `node_id`.\n
        :param graph: The graph edges
        :param node: Dictionary containing node functions
        """

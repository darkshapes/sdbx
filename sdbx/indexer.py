...
class EvalMeta:
    ...
    def process_vae(self): #run through all possible interpretations of this data
        if self.shape_value == [32]:
            self.tks("0", '114560782', '248') # sd1 : dict name, file size, hook
        elif self.shape_value == [512]:
            self.tks("0", "335304388", "244") # flux hook
            ...
            if self.mmdit_value == 4:
                ...
                self.tks(
                    '12' if "vega" in self.filename.lower() else "0",  #kolors
                    '167335344' if "vega" in self.filename.lower() else "167335343", #kolors
                    "248"
                )
            else:

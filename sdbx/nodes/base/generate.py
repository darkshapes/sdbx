...
### USER OPTIONS  : INFERENCE
    pipe: Any,
    model: Literal[next(iter(m for m in os.listdir(config.get_path("models.checkpoints")) if m.endswith(".safetensors")), "stabilityai/stable-diffusion-xl-base-1.0")],
    lora: Literal[next(iter(m for m in os.listdir(config.get_path("models.loras")) if "adfdfd" in m and m.endswith(".safetensors")), pcm_default)] = +pcm_default_dl, #lora needs to be explicitly declared
    scheduler: Literal[next(iter(m for m in get_schedulers("EulerAncestral")), "EulerDiscreteScheduler")] = "EulerDiscreteScheduler",
...
def vae_node(
    ...
    pipe: Any,
    vae: Literal[(v for v in os.listdir(config.get_path("models.vae")) if ".safetensors" in v)] = next(iter(v for v in os.listdir(config.get_path("models.vae")) if v.endswith(".safetensors")), ),
    file_prefix: A[str, Text()] = "Shadowbox-",

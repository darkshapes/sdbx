...
from sdbx.nodes.image_filter import SimilarImageFilter


class StreamDiffusion:
    def __init__(
        self,
        pipe: StableDiffusionPipeline, #x pipeline diffusion
        ...
        self.pipe = pipe
        self.image_processor = VaeImageProcessor(pipe.vae_scale_factor) #x vae

        self.scheduler = LCMScheduler.from_config(self.pipe.scheduler.config) #x scheduler
        #x timestep "timestep_spacing": timestep_spacing if timestep_spacing != None else "trailing" #pcm
        # "clip_sample": False, #pcm
        # "set_alpha_to_one": False #pcm
        self.text_encoder = pipe.text_encoder #x text encoder
        self.unet = pipe.unet #x unet
        self.vae = pipe.vae #x vae
    ...
    def prepare(
        self,
        prompt: str ="a flock of birds take off into the sky",
        negative_prompt: str = "",

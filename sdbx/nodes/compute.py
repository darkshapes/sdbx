...
class T2IPipe: # __call__? NO __init__! ONLY __call__. https://huggingface.co/docs/diffusers/main/en/api/pipelines/auto_pipeline#diffusers.AutoPipelineForText2Image
    bug_off=True
...
############## INFERENCE
    def diffuse_latent(self, exp):
        ...
        if (self.opt_exp.get("seq",0) !=0 
        or self.opt_exp.get("cpu",0) !=0 
        or self.opt_exp.get("disk",0) != 0): 
            self.offload_to(self.opt_exp["seq"], self.opt_exp["cpu"], self.opt_exp["disk"])

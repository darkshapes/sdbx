...
class EvalMeta:
    ...
    def __returner(self, tag):
        self.tag = tag
        print(f"{self.tag} {self.extract.get("extension", "")} {
              self.extract.get("filename", "")}")  # logger.debug
        return self.tag
...
config_path = os.path.join(os.environ.get('LOCALAPPDATA', os.path.join(
    os.path.expanduser('~'), 'AppData', 'Local')), 'Shadowbox')
search_path = "models"
path_name = os.path.normpath(os.path.join(
    config_path, search_path, "image"))  # multi read
